...
| File | Description |
|---|---|
| `Q1_FCFS.cpp` | First Come First Serve scheduling |
| `Q2_SJF.cpp` | Non-Preemptive Shortest Job First scheduling |
| `Q3_Priority.cpp` | Non-Preemptive Priority scheduling |
| `Q4_RoundRobin.cpp` | Round Robin scheduling |
| `Q5_Comparison.cpp` | Compares FCFS, SJF, and Priority on the same data set |
...
```bash
g++ -std=c++17 -O2 -o Q1_FCFS Q1_FCFS.cpp
./Q1_FCFS
```
...
```bash
g++ -std=c++17 -O2 -o Q2_SJF Q2_SJF.cpp
./Q2_SJF
...
- **Note:** SJF is provably optimal for minimizing average waiting time
  when all processes arrive at time 0, so it will always be the best or
  tied-best result unless Priority scheduling happens to produce the exact
